...
public class SrsEncoder {
    ...
    //初始化了音视频的mediaCodec。负责开启音视频编码器。相当于调用了mediaCodec的start()
    ...
    /**
     * 个人推断这个方法会在摄像头捕获到数据后回调。属于硬编码
     * 跟liang.zhou.lane8.no5.my_push.cameraLib.VideoEncoder的作用类似
     * @param yuvFrame 摄像头的原始数据。因为camera捕获到的数据格式为yuv
     * @param pts 告诉解码器何时显示这一帧数据
     */
    ...
    //es即为mediaCodec编码成h264的视频输出。个人推断作用是将视频封装成flv或者MP4格式
    ...
    /**
     * 个人推断这个方法会在麦克风捕获到数据后回调。属于硬编码
     * @param data 麦克风的原始音频数据。因为麦克风捕获到的数据格式为pcm
     * @param size
     */
